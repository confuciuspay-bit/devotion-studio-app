// Fee engine — implements 11_fees.md exactly

export interface FeeBreakdown {
  pct: number;       // % portion (USD)
  fixed: number;     // fixed fee (USD)
  total: number;
  net: number;       // amount - total
  rate: number;      // applied rate (decimal, e.g. 0.0050)
  label: string;
}

// PSP marginal tiers from 11_fees.md
...
// Marginal fee on a single payment given monthly volume so far
...
// Vault is flat 2% all-in (PSP free)
...
// Wallet swap spread (depending on pair liquidity)
